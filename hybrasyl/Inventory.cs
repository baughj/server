    ...
    public class Exchange
    {
        ...
        public static bool StartConditionsValid(User source, User target)
        {
            return source.Map == target.Map && source.IsInViewport(target) &&
                   ...
                   source.Status == Enums.StatusFlags.Alive &&
                   target.Status == StatusFlags.Alive && target.Distance(source) <= Constants.EXCHANGE_DISTANCE;
        ...
        public bool ConditionsValid
        {
            get
            {
                return _source.Map == _target.Map && _source.IsInViewport(_target) &&
                       ...
                       _source.Status.HasFlag(StatusFlags.InExchange) &&
                       _target.Status.HasFlag(StatusFlags.InExchange) &&
                       _active;
            }
        }
        ...
        public bool StartExchange()
        {
            ...
            _active = true;
            _source.Status |= StatusFlags.InExchange;
            _target.Status |= StatusFlags.InExchange;
